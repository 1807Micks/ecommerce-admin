...
export const createCheckoutSession = async (req, res) => {
  try {
    ...
    const lineItems = products.map((product) => {
      const amount = Math.round(product.price * 100); //stripe want you to send in cents
  ...
  try {
    ...
    if (session.payment_status === "paid") {
      ...
      res.statue(200).json({
        success: true,
        message:
          "Payment sucessful, order created and coupon deactivated if used",
